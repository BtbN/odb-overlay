...
DESCRIPTION="Codesynthesis' ODB runtime library for accessing a SQLite database"
...
DEPEND="=dev-db/libodb-${MY_V}*[static-libs?,threads=]
		threads? ( dev-db/sqlite[static-libs?,threadsafe(+),unlock-notify(+)] )
	   !threads? ( dev-db/sqlite[static-libs?,threadsafe(-),unlock-notify(-)] )"
...
src_configure() {
	#--{en,dis}able-static will be passed by autotools-utils 
	myeconfargs=(
		--docdir="${T}"
		$(use_enable threads)
	)
